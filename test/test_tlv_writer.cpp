...
using SubStructArray3 = SubStruct[3];

DEFINE_STRUCT_WITH_TUPLE_INTERFACE(ParentStructWithArray,
    (SubStructArray3, subDataArray)
);

TEST_F(TLVWriterTest, SubStructTLVConverter_Array) {
    constexpr uint32_t SUB_STRUCT_TYPE = 0x9001;
    constexpr uint32_t SUB_INT_TYPE = 0x9002;
    constexpr uint32_t SUB_DOUBLE_TYPE = 0x9003;
    ...
    ParentStructWithArray parentStructWithArray;
    parentStructWithArray.subDataArray[0].intField = 123;
    parentStructWithArray.subDataArray[0].doubleField = 4.56;
    parentStructWithArray.subDataArray[1].intField = 789;
    parentStructWithArray.subDataArray[1].doubleField = 1.23;
    parentStructWithArray.subDataArray[2].intField = 456;
    parentStructWithArray.subDataArray[2].doubleField = 7.89;

    StructFieldsConvert(parentStructWithArray, sharedWriter, parentMappingTuple);

    // 验证数据
    const uint8_t* data = sharedWriter->data();
    ASSERT_NE(data, nullptr);
    
    // 预期结构：3个外层TLV，每个包含内部的两个TLV
    // 单个内部数据大小：(int TLV: 4+4+4) + (double TLV: 4+4+8) = 28 字节
    // 单个外层数据大小：4+4+28 = 36 字节
    // 总大小：36 * 3 = 108 字节
    size_t expectedInnerSize = 2 * (2 * sizeof(uint32_t)) + sizeof(int32_t) + sizeof(double);
    size_t expectedSingleOuterSize = 2 * sizeof(uint32_t) + expectedInnerSize;
    size_t expectedTotalSize = expectedSingleOuterSize * 3;
    EXPECT_EQ(sharedWriter->size(), expectedTotalSize);
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        int32_t actualIntValue;
        memcpy(&actualIntValue, data + offset, sizeof(int32_t));
        EXPECT_EQ(actualIntValue, parentStructWithArray.subDataArray[i].intField);
        offset += sizeof(int32_t);
        ...
        double actualDoubleValue;
        memcpy(&actualDoubleValue, data + offset, sizeof(double));
        EXPECT_DOUBLE_EQ(actualDoubleValue, parentStructWithArray.subDataArray[i].doubleField);
        offset += sizeof(double);
    }
}